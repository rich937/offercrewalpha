...
export default function Dashboard() {
  ...
  const analyzeWithCrew = async () => {
    ...
    try {
      ...
      const lines = (result.crewResponse || "Crew responded").split('\n').filter((l: string) => l.trim().length > 5);
      const crewMessages = lines.map((line: string) => {
        let clean = line.trim();
        // Remove duplicate speaker name if present
        clean = clean.replace(/^(Ledger|Shade|Spark|Clara):\s*/i, '');
        let type = 'spark';
        ...
        <div className="max-w-7xl mx-auto px-6 py-8 flex gap-8 h-[calc(100vh-180px)]">
                ...
                <div className="flex-1 p-6 overflow-y-auto bg-gray-50 space-y-6" style={{ maxHeight: '520px' }}>
                  {chatMessages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.type === 'user' ? 'justify-end' : 'items-start gap-3'}`}>
                      {msg.type !== 'user' && <img src={getIconPath(msg.type)} alt={msg.type} className="w-11 h-11 rounded-2xl mt-1" />}
          ...
          <div className="w-80 flex-shrink-0">
            ...
            <div className="space-y-3 overflow-y-auto pr-2" style={{ maxHeight: '620px' }}>
              ...
              {history.map((offer, i) => (
                <div key={i} className="bg-white border border-gray-200 rounded-2xl p-5 hover:border-cyan-400 cursor-pointer">
