...
export default function Dashboard() {
  ...
  const sendUserMessage = async () => {
    ...
    try {
      ...
      if (data.crewResponse) {
        ...
        const crewMsgs = lines.map((line: string) => ({ type: 'spark', text: line.trim() }));
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-2xl">Loading Dashboard...</div>;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-6 border-b bg-white">
        <div className="flex gap-10 text-lg font-medium">
          ...
          <Link href="/blog" className="pb-4 border-b-2 border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300">Blog / Podcasts</Link>
            ...
            {selectedFiles.length > 0 && (
              <div className="mt-6">
                ...
                <button 
                  ...
                  disabled={uploading}
                  className="w-full py-4 bg-gradient-to-r from-cyan-500 to-purple-600 text-white rounded-2xl font-semibold hover:brightness-110 disabled:opacity-50"
                >
                ...
                <div className="flex-1 p-6 overflow-y-auto bg-gray-50 space-y-6" style={{ maxHeight: '520px' }}>
                  {chatMessages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.type === 'user' ? 'justify-end' : 'items-start gap-3'}`}>
                      ...
                      {msg.type === 'user' && (
                        <div className="w-11 h-11 rounded-2xl bg-cyan-600 text-white flex items-center justify-center font-bold mt-1">
                          {getUserInitial()}
                        </div>
                      )}
